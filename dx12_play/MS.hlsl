	...
	out indices uint3 outIndices[MAX_PRIMS])
{
	...
	if (groupThreadId < vertCount)
	{
		...
		outVerts[groupThreadId].posW = mul(Vertices[vertexIndex].pos.xyz, Instances[InstanceOffset + instanceIndex].gWorld)+ Instances[InstanceOffset + instanceIndex].gWorld[3].xyz;
